...
print("-V- Scipy Version = {:}".format(scipy.__version__))
import numpy as np


def getNumberOfBins(N, dim=1, rule="sturges", L=1.0):
    ######################
    ## Diakonis rule
    ######################
    # IQR = 1
    ...
    if rule == "sturges":
        ...
        N_eff = np.rint(np.power(N, 1.0 / dim))
        nbins = int(1 + np.log2(N_eff))
    elif rule == "rice":
        ######################
        ## Rice rule
        ######################
        N_eff = np.rint(np.power(N, 1.0 / dim))
        nbins = 2.0 * np.power(N_eff, 1.0 / 3.0)
...
def evaluateWassersteinDistance(data1, data2):
    ...
    elif len(np.shape(data1)) == 2 and np.shape(data1)[1] == 1:
        error = stats.wasserstein_distance(data1[:, 0], data2[:, 0])
...
def get_density(positions, nbins, bounds):
    ...
    elif (len(np.shape(positions)) == 2) and (np.shape(positions)[1] == 1):
        # 1-D case (channels)
...
def evaluateL1HistErrorVector(data1, data2, nbins, bounds):
    ...
    if len(np.shape(data1)) == 1:
        ...
        ComputeError = simps(error_vec[0], grid_centers1[0])
