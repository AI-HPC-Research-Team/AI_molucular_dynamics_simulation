...
def computeLatentDynamicsDistributionErrorAlanine(model, set_name):
    print("# computeLatentDynamicsDistributionErrorAlanine() #")
        ...
        if len(idx_region) > 0:
            ...
            for log_norm in [False]:
                ...
                for with_colorbar in [True]:
                    ...
                    textfontsize=12
                    phi = data_all_region[:, 20] * 180 / np.pi
                    psi = data_all_region[:, 21] * 180 / np.pi
                    if len(phi) > 0:
